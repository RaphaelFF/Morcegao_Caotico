...
def jogo_principal(TELA, RELOGIO_FPS, IMAGENS, SONS, MASCARAS_COLISAO, info_movimento):
    ...
    while True:
        ...
        # Lógica da Pontuação
        centro_passaro = posicao_passaro_x + IMAGENS['jogador'][0].get_width() / 2
        for cano in canos_superiores:
            centro_cano = cano['x'] + IMAGENS['cano'][0].get_width() / 2
            if centro_cano <= centro_passaro < centro_cano + 4:
                pontuacao += 1
                SONS['ponto'].play()
